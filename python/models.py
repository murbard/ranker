...
def bump_eval(A, P, Q, μδ, σδ, order=2):
    ...
    if order == 0:
        T = 0.0
        for a, p, q in zip(A, P, Q):
            D = 1 + 2*a*s
            T += exp(-a*μ*μ/D)/sqrt(D)*(p + q*(a*μ*μ/(D*D) + a*s/D))
...
class TieModel():
    ...
    and λ = 0 recovers Bradley-Terry.  The variational posterior over λ is categorical over J
    fixed atoms placed at prior quantiles of p0 = λ/(2+λ) (each with prior mass 1/J); the atom
    file is generated by python/fit_tie_atoms.py.  Each atom's bump
        b_λ(δ) = log(1 + λ/(2 cosh(δ/2)))
    is represented in a shared basis {e^{-a_l δ²}, a_l δ² e^{-a_l δ²}} whose Gaussian integrals
    ...
    def __init__(self, path=None):
        ...
        with open(path) as f:
            ...
            self.J, self.L = J, L
            self.a = array([float(x) for x in f.readline().split()])
            assert len(self.a) == L
            self.λ = empty(J); self.π = empty(J)
            ...
            for j in range(J):
                ...
                self.P[j] = vals[2::2]
                self.Q[j] = vals[3::2]
        self.logλ = log(self.λ)
        self.w = ones(J) / J
        self.premix()

    def premix(self):
        """Mix the per-atom coefficients by the current weights (used by eval's single kernel call)."""
        self.pmix = self.w @ self.P
        self.qmix = self.w @ self.Q
        self.wlogλ = self.w @ self.logλ
...
class Instance():
    ...
    def observe_ties(self, n_obs, λ):
        """
        Samples n_obs Davidson comparisons (true tie parameter λ).
        Returns (wins, ties): a coo_matrix of directed win counts and an upper-triangular
        coo_matrix of tie counts.
        """
        wins = zeros((self.n, self.n)); ties = zeros((self.n, self.n))
        ps = empty(self.n * (self.n - 1) // 2)
        ps.fill(1.0 / len(ps))
        counts = rng.multinomial(n_obs, ps)
        k = 0
        for i in range(self.n):
            for j in range(i + 1, self.n):
                if counts[k] > 0:
                    ...
                    Z = 2 * np.cosh(d / 2) + λ
                    pw, pt = exp(d / 2) / Z, λ / Z
                    nw, nt, nl = rng.multinomial(counts[k], [pw, pt, max(1 - pw - pt, 0)])
                    wins[i, j] = nw; wins[j, i] = nl; ties[i, j] = nt
                k += 1
        return coo_matrix(wins), coo_matrix(ties)

    def match3(self, i, j, λ):
        """
        One Davidson comparison between i and j: returns +1 (i wins), 0 (tie), -1 (j wins).
        """
        d = self.z[i] - self.z[j]
        r = rng.random() * (2 * np.cosh(d / 2) + λ)
        return 1 if r < exp(d / 2) else (0 if r < exp(d / 2) + λ else -1)
...
class VBayes():
    ...
    def __init__(self, model):
        ...
        self.n = model.n
        self.ties = None   # set via enable_ties() for the Davidson tie model
    ...
    def eval(self, obs, compute_gradient = False, compute_hessian = False, dobs = None, tie_obs = None, pairs = None):
        ...
        # --- Davidson ties ---------------------------------------------------------------
        # Relative to the Bradley-Terry term above (n_w F(μδ) + n_l F(-μδ) per pair), the tie
        # model adds, for each unordered pair (a<b) with counts (n_w, n_l, n_t), c = n_w+n_l+n_t:
        #     n_t F(μδ,σδ)  +  c E[b̄](μδ,σδ)  +  n_t μδ/2  -  n_t Σ_j w_j log λ_j
        # where F is the same erf+correction kernel (every outcome pays the log-normalizer
        # E[log(2cosh(δ/2)+λ)] = μδ/2 + F + E[b_λ]) and E[b̄] is the premixed atom bump.
            ...
            for (a, b), (n_w, n_l, n_t) in pairs.items():
                ...
                if compute_gradient:
                    ...
                    if compute_hessian:
                        ...
                        gh.h_diag[a] += Tμμ
                        gh.h_diag[b] += Tμμ
                        gh.h_diag[self.n + a] += Tσσ * (σ[a]/σδ)**2 + Tσ * σ[b]**2 / σδ**3
                        gh.h_diag[self.n + b] += Tσσ * (σ[b]/σδ)**2 + Tσ * σ[a]**2 / σδ**3
                        # off-diagonal entries (a < b throughout, so row < col)
                        for (r, cc, val_) in ((a, b, -Tμμ),
                                              (n + a, n + b, Tσσ * σ[a]*σ[b]/σδ**2 - Tσ * σ[a]*σ[b]/σδ**3),
                                              (a, n + a,  Tμσ * σ[a]/σδ),
                                              (a, n + b,  Tμσ * σ[b]/σδ),
                                              (b, n + a, -Tμσ * σ[a]/σδ),
                                              (b, n + b, -Tμσ * σ[b]/σδ)):
                            gh.h_obs.data[k] = val_
                            gh.h_obs.row[k] = r
                            gh.h_obs.col[k] = cc
                            k += 1
    ...
    def cavi(self, obs, tie_obs=None, pairs=None):
        ...
        The per-pair basis integrals are accumulated once (L values); each atom is then an
        L-dot, so the pass over pairs is J-free.  Returns max |Δw_j|.
        """
